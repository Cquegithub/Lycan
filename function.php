...
function check_idcard($id) {
	$id = strtoupper($id); // 转换成大写
	$regx = "/(^\d{15}$)|(^\d{17}([0-9]|X)$)/"; // 正则
	$arr_split = array();
	...
	if(strlen($id) == 15) { // 早先的15位的身份证号
		...
		if(!strtotime($dtm_birth)) { // 检查生日日期是否正确
			...
			// 检查18位身份证的校验码是否正确
			// 校验码按照ISO 7064:MOD 11-2的规定生成，X可以认为是数字10
			
			// 加权因子
			$factor = array(7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2);
			
			// 校验码对应值
			$verify_num_list = array('1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2');
			
			$checksum = 0;
			
			for($i = 0; $i < 17; $i++) {
				$checksum += (int)$id{$i} * $factor[$i];
			}
			
			$mod = $checksum % 11;
			$val_num = $verify_num_list[$mod];
